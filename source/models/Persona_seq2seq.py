...
from source.modules.encoders.self_inf_encoder import Self_inf_Encoder, ISIEncoder
...
class PersonaSeq2Seq(BaseModel):
    """
    PersonaSeq2Seq
    """
    def __init__(self, src_vocab_size, tgt_vocab_size, embed_size, hidden_size, padding_idx=None,
        ...
        super(PersonaSeq2Seq, self).__init__()
        ...
        # self.word_encoder = RNNEncoder(input_size=self.embed_size, hidden_size=self.hidden_size,
        #                           embedder=enc_embedder, num_layers=self.num_layers,
        #                           bidirectional=self.bidirectional, dropout=self.dropout)
        # self.utterance_encoder = RNNEncoder(input_size=self.hidden_size, hidden_size=self.hidden_size,
        #                           num_layers=self.num_layers,
        #                           bidirectional=self.bidirectional, dropout=self.dropout)
        # self.encoder = HRNNEncoder(self.word_encoder, self.utterance_encoder)

        self.word_encoder = Self_inf_Encoder(input_size=self.embed_size, hidden_size=self.hidden_size,
                                  embedder=enc_embedder, num_layers=self.num_layers,
                                  bidirectional=self.bidirectional, dropout=self.dropout)
        self.utterance_encoder = RNNEncoder(input_size=self.hidden_size+self.num_kers*3, hidden_size=self.hidden_size,
                                  num_layers=self.num_layers,
                                  bidirectional=self.bidirectional, dropout=self.dropout)
        self.encoder = ISIEncoder(self.word_encoder, self.utterance_encoder)
    ...
    def encode(self, inputs, hidden=None, is_training=False):
        ...
        if self.use_posterior:
            ...
            # tgt_enc_inputs = inputs.src[0][:, -1, 1:-1], inputs.src[1]-2
    ...
    def collect_metrics(self, outputs, target, epoch=-1, target_lens=None):
        """
        collect_metrics
        """
        ...
        ppl_v2 = (torch.exp(self.nll_loss(logits, target, reduction=False) / target_lens.float())).mean()  ## 一个batch中ppl(set)的平均
        metrics.add(ppl_v2=ppl_v2)
        if self.use_posterior:
    ...
    def collect_metrics_pg(self, outputs, target, rewards, epoch=-1):
        """
        collect_metrics
        """
        num_samples = target.size(0)
        ...
        pg_loss = (self.nll_loss(logits, target, reduction=False) * rewards).sum()
        loss += pg_loss
        metrics.add(pg_loss=pg_loss)
    ...
    def iterate(self, inputs, optimizer=None, grad_clip=None, is_training=False, epoch=-1):
        ...
        target_lens = inputs.tgt[1] - 1
        outputs = self.forward(enc_inputs, dec_inputs, is_training=is_training)
        metrics, scores = self.collect_metrics(outputs, target, epoch=epoch, target_lens=target_lens)
    ...
    def iterate_pg(self, inputs, rewards, optimizer=None, grad_clip=None, is_training=False, epoch=-1):
        """
        iterate
        """
        enc_inputs = inputs
        ...
        outputs = self.forward(enc_inputs, dec_inputs, is_training=is_training)
        metrics, scores = self.collect_metrics_pg(outputs, target, rewards, epoch=epoch)
        loss = metrics.loss
        if torch.isnan(loss):
            raise ValueError("nan loss encountered")
