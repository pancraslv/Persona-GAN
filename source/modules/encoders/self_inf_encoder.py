...
class Self_inf_Encoder(nn.Module):
    """
    ...
    def __init__(self,
                 ...
                 bidirectional=True,
                 dropout=0.0):
        super(Self_inf_Encoder, self).__init__()
        ...
        self.num_kers = 32
        self.conv1 = nn.Conv2d(1, self.num_kers, 2)
        self.conv2 = nn.Conv2d(1, self.num_kers, 3)
        self.conv3 = nn.Conv2d(1, self.num_kers, 4)
        self.pool = nn.MaxPool2d(2, 2)
        self.cnn_ln = nn.LayerNorm(self.num_kers*3)
        self.rnn_ln = nn.LayerNorm(self.hidden_size)
    ...
    def forward(self, inputs, hidden=None):
        ...
        cnn_inputs = rnn_inputs.unsqueeze(1)

        cnn_output1 = (self.pool(self.conv1(cnn_inputs))).sum(dim=-1).sum(dim=-1)
        cnn_output2 = (self.pool(self.conv2(cnn_inputs))).sum(dim=-1).sum(dim=-1)
        cnn_output3 = (self.pool(self.conv3(cnn_inputs))).sum(dim=-1).sum(dim=-1)

        cnn_output = torch.cat([cnn_output1, cnn_output2, cnn_output3], dim=-1)
        ...
        rnn_output = last_hidden[-1].view(batch_size, -1)

        final_output = torch.cat([self.cnn_ln(cnn_output), self.rnn_ln(rnn_output)], dim=-1)
        # final_output = torch.cat([cnn_output, rnn_output], dim=-1)
        return final_output
...
class ISIEncoder(nn.Module):
    ...
    def __init__(self,
                 sub_encoder,
                 hiera_encoder):
        super(ISIEncoder, self).__init__()
        self.sub_encoder = sub_encoder
        self.hiera_encoder = hiera_encoder
        self.fc1 = nn.Linear(self.hiera_encoder.hidden_size, 1)
        self.fc2 = nn.Linear(self.hiera_encoder.hidden_size, 1)
    ...
    def forward(self, inputs, features=None, sub_hidden=None, hiera_hidden=None,
        ...
        sub_hidden = self.sub_encoder(sub_enc_inputs, sub_hidden)
        ...
        #
        # H_A = hiera_outputs[:, ::2, :]
        # H_B = hiera_outputs[:, 1::2, :]
        #
        # if H_B.shape[1]+1 == H_A.shape[1]:
        #     pad = torch.zeros(size=(batch_size, 1, 800), dtype=torch.float).cuda()
        #     H_B = torch.cat([H_B, pad], dim=1)
        # H = H_A * H_B
        # u_a = self.fc1(H).squeeze()
        # alpha_b2a = self.softmax(u_a)
        #
        # u_b = self.fc2(H).squeeze()
        # alpha_a2b = self.softmax(u_b)
        #
        # H_A_hat = H_A * (alpha_b2a.unsqueeze(2).repeat(1, 1, 800))
        # H_B_hat = H_B * (alpha_a2b.unsqueeze(2).repeat(1, 1, 800))
        #
        # H_hat = torch.zeros(size=(batch_size, max_hiera_len, 800), dtype=torch.float).cuda()
        # H_hat[:, ::2, :] = H_A_hat
        # H_hat[:, 1::2, :] = H_B_hat[:, :-1, :]
        ...
        # return H_hat, hiera_hidden, None
        return hiera_outputs, hiera_hidden, None
