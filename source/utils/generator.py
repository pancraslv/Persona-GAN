...
from source.utils.misc import sequence_mask, list2tensor_with_ml
...
class TopKGenerator(object):
    ...
    def generate_batch(self, batch):
        ...
        generate
        {
            'cue': ['your persona: i read twenty books a year.',
                    "your persona: i'm a stunt double as my second job.",
                    'your persona: i only eat <unk>',
                    'your persona: i was raised in a single parent <unk>'],
            'src': 'hello what are doing today ?',
            'tgt': 'i am good , i just got off work and tired , i have two jobs .',
            'preds': ['hi , i am doing great . how are you ?'],
            'scores': [-0.8839994668960571]
        }
        """
        # results = []
        # batch_cnt = 0
    ...
    def greedy_generate_batch(self, batch):
        """
        generate
        batch_iter = valid_iter
        """

        dec_init_state = (self.model.encode(batch))[-1]
        dec_inputs = batch.tgt[0][:, :-1], batch.tgt[1] - 1
        max_len = batch.tgt[0].shape[-1] - 2
        prediction = self.model.decoder.eval_(dec_inputs, dec_init_state, max_length=max_len)

        return prediction

    def generate_greedy(self, batch, use_tf=False):
        """
        generate
        此处可加teacher forcing
        """

        preds = self.greedy_generate_batch(batch)
        ...
        preds = self.tgt_field.denumericalize(preds)
        if 'cue' in batch:
            cue = [' '.join(i) for i in self.tgt_field.denumericalize(batch.cue[0].data)]
        result_batch = []
        result_batch_for_dis = []
        for i in range(len(src)):
            result_batch.append(['0', src[i], preds[i], cue[i]])
            result_batch_for_dis.append(['0', src[i], preds[i], cue[i]])
        for i in range(len(src)):
            result_batch_for_dis.append(['1', src[i], tgt[i], cue[i]])
        # batch.tgt = list2tensor(self.src_field.numericalize(preds))
        # batch['tgt'] = list2tensor(self.src_field.numericalize(preds))
        gen_inputs = Pack()
        if use_tf:
            gen_inputs['src'] = (batch.src[0].repeat(2, 1, 1), batch.src[1].repeat(2, 1))
            temp = list2tensor_with_ml(self.src_field.numericalize(preds), batch.tgt[0].shape[-1])
            gen_inputs['tgt'] = (torch.cat([temp[0].cuda(), batch.tgt[0]], dim=0), torch.cat([temp[1].cuda(), batch.tgt[1]], dim=0))
            gen_inputs['cue'] = (batch.cue[0].repeat(2, 1, 1), batch.cue[1].repeat(2, 1))
            return result_batch_for_dis, gen_inputs.cuda(), result_batch_for_dis
        else:
            gen_inputs['src'] = batch.src
            gen_inputs['tgt'] = list2tensor(self.src_field.numericalize(preds))
            gen_inputs['cue'] = batch.cue
            return result_batch, gen_inputs.cuda(), result_batch_for_dis