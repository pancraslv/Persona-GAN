...
def corpus_bleu(hyps, refs):
    ...
    bleu_1 = bleu_score.corpus_bleu(refs, hyps, weights=(1, 0, 0, 0), smoothing_function=SmoothingFunction().method0)
    bleu_2 = bleu_score.corpus_bleu(refs, hyps, weights=(0.5, 0.5, 0, 0), smoothing_function=SmoothingFunction().method0)
    # for hyp, ref in zip(hyps, refs):
    #     try:
    #         score = bleu_score.sentence_bleu(
    #             [ref], hyp,
...
def corpus_Knowledge_R_P_F1(hyps, cues):
    sw_counter = Counter()
    for line in open('./data/stopwords.txt', 'r', encoding='utf-8'):
        sw_counter[line.strip()] += 1
    cue_counter = Counter()
    hyp_counter = Counter()
    for hyp, cue in zip(hyps, cues):
        for line in cue:
            cue_counter.update(line.strip().split(' '))
        hyp_counter.update(hyp)

    cue_counter -= sw_counter
    hyp_counter -= sw_counter
    R = sum((cue_counter & hyp_counter).values()) / sum(cue_counter.values())
    P = sum((cue_counter & hyp_counter).values()) / sum(hyp_counter.values())
    F1 = 2 * R * P / (R + P)
    return R, P, F1
