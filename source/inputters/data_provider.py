...
class DataProcessor(object):
    ...
    def preprocessing_for_one_batch(self, sth):
        examples = self._create_examples(sth, "test")
        self.num_examples['test'] = len(examples)
        def instance_reader():
            """ instance reader """
            for (index, example) in enumerate(examples):
        ...
        for instance in instance_reader():
            batch.append(instance)
        batch = self.generate_batch_data(batch, device=0)
        return batch
...
class MatchProcessor(DataProcessor):
    ...
    def get_train_examples(self, data_dir):
        ...
        return self._create_examples(
            self._read_data(os.path.join(data_dir, "dis_train.txt")), "train")

    def get_dev_examples(self, data_dir):
        """See base class."""
        return self._create_examples(
            self._read_data(os.path.join(data_dir, "dis_dev.txt")), "dev")

    def get_test_examples(self, data_dir):
        """See base class."""
        return self._create_examples(
            self._read_data(os.path.join(data_dir, "dis_test.txt")), "test")
