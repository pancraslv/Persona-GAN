...
class PersonaCorpus(Corpus):
    ...
    def __init__(self,
        ...
        self.filter_pred = lambda ex: tgt_filter_pred(ex['tgt'])
        ...
        with open(data_file, "r", encoding="utf-8") as f:
            for line in f:
                if self.with_label:
                    ...
                    src_list = []
                    for sent in src.split('[u_sep]'):
                        src_list.append(' '.join(sent.split()[:self.max_len]))

                    filter_knowledge = []
                    for sent in knowledge.split('[p_sep]'):
                        filter_knowledge.append(' '.join(sent.split()[:self.max_len]))

                    data.append({'src': src_list, 'tgt': tgt, 'cue':filter_knowledge})
